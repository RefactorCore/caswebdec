...
def balance_sheet():
    ...
    revenues = {code: -to_decimal(bal) for code, bal in is_agg.items() if Account.query.filter_by(code=code, type='Revenue').first()}
    expenses = {code: to_decimal(bal) for code, bal in is_agg.items() if Account.query.filter_by(code=code, type='Expense').first()}
...
def export_balance_sheet():
    ...
    revenues = {code: -to_decimal(bal) for code, bal in is_agg_net_income.items() if Account.query.filter_by(code=code, type='Revenue').first()}
    expenses = {code: to_decimal(bal) for code, bal in is_agg_net_income.items() if Account.query.filter_by(code=code, type='Expense').first()}
    # --- END MODIFICATION ---
...
def aggregate_account_balances(start_date=None, end_date=None):
    ...
    Balances are returned as Decimal quantized to 2dp.
    """
    ...
    # return plain dict of Decimal values
    return dict(agg)
...
def general_ledger():
    ...
    for acc_code, balance in agg.items():
        ...
        if is_debit_normal:
            ...
            final_debit = balance if balance >= Decimal('0.00') else Decimal('0.00')
            final_credit = abs(balance) if balance < Decimal('0.00') else Decimal('0.00')
            ...
            final_debit = balance if balance > Decimal('0.00') else Decimal('0.00')
            final_credit = abs(balance) if balance < Decimal('0.00') else Decimal('0.00') # ✅ FIXED
